...
fun intersect (v1,v2) =
    case (v1,v2) of
      ...
      | (VerticalLine x1, VerticalLine x2) => 
	  		if real_close(x1,x2) then v1 (* same line *) else NoPoints (* parallel *)
...
(* CHANGE: Add a case for Shift expressions *)
	  | Shift(v1, v2, e1) => 
			  ...
			  | VerticalLine x => VerticalLine (x + v1)
			  | LineSegment (x1, y1, x2, y2) => LineSegment (x1 + v1, y1 + v2, x2 + v1, y2 + v2)
